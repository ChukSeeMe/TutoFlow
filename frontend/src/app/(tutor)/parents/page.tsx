...
function ParentRow({ parent }: { parent: ParentGuardian }) {
  return (
    <Link
      ...
      className="bg-white rounded-xl border border-gray-200 px-5 py-4 flex items-center justify-between hover:shadow-sm hover:border-brand-200 transition-all group"
    >
      <div className="flex items-center gap-3 min-w-0">
        ...
        <div className="min-w-0">
          <p className="font-medium text-gray-900 group-hover:text-brand-700 transition-colors">{parent.full_name}</p>
      ...
      <div className="flex items-center gap-4 flex-shrink-0">
        ...
        <span className="text-xs text-gray-400">
          {parent.linked_student_ids?.length ?? 0} student{(parent.linked_student_ids?.length ?? 0) !== 1 ? "s" : ""}
        </span>
